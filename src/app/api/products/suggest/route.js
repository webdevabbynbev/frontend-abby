...
export async function GET(req) {
  try {
    ...
    const getProductBrandSlug = (product) =>
      String(
        product?.brand?.slug || product?.brand_slug || product?.brandSlug || "",
      ).toLowerCase();

    const getProductBrandId = (product) =>
      String(product?.brand_id || product?.brandId || product?.brand?.id || "");
    ...
    const matchesKeyword = (product) => {
      ...
      if (matcher(name) || matcher(sku) || matcher(brand) || matcher(category))
        return true;
    ...
    const matchesBrand = (product) => {
      ...
      const productBrandSlug = normalizeSearchText(
        getProductBrandSlug(product),
      );
